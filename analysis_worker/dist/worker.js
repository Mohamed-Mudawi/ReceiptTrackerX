...
if (!queueName)
    throw new Error("Missing SERVICE_BUS_QUEUE_NAME");
if (!cosmosEndpoint)
    throw new Error("Missing COSMOS_ENDPOINT");
if (!cosmosKey)
    throw new Error("Missing COSMOS_KEY");
if (!cosmosDatabaseName)
    throw new Error("Missing COSMOS_DATABASE_NAME");
if (!cosmosContainerName)
    throw new Error("Missing COSMOS_CONTAINER_NAME");
...
async function updateMonthlySummary(message) {
    ...
    try {
        const { resource: existingSummary } = await summaryRef.read();
        if (existingSummary) {
            const processedReceiptIds = existingSummary.processed_receipt_ids ?? [];
            if (processedReceiptIds.includes(message.receiptId)) {
                console.log(`Receipt ${message.receiptId} already counted. Skipping update.`);
                return;
            }
